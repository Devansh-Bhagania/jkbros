        ...
        const handleNextClick = async () => {
            if (phoneNumber && /^\d{10}$/.test(phoneNumber)) {
              try {
                ...
                const confirmationResult = await signInWithPhoneNumber(authentication, `${selectedCountryCode}${phoneNumber}`, appVerifier);
    ...
    return (
        <div className='flex-1 md:bg-gray-200 bg-white md:py-4 md:px-20 pt-1'>
            <div className='container sm:p-10 mx-auto md:mb-2 md:mt-1 xl:mx-14 bg-white p-6  rounded-lg md:p-10 h-full w-auto'>
                <h1 className='  text-textPrimary mb-5 font-playfair text-2xl'>Welcome</h1>
                <h4 className='mb-1 text-lg font-semibold'>Create a new account</h4>
                <p className='text-left  md:text-sm mb-12  text-zinc-400'>With a valid number, you can access deliveries, and our other services.</p>
